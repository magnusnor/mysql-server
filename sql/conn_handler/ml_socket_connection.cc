...
bool MLSocket::ConnectSocket() {
  ...
  strncpy(address.sun_path, m_socket_path.c_str(),
          sizeof(address.sun_path) - 1);

  for (int i = 0; i < max_retries; ++i) {
    if (connect(m_socket_fd, (struct sockaddr *)&address, sizeof(address)) ==
        0) {
...
void MLQueryRep::AddPredicates(JoinHypergraph *graph, const JoinPredicate *edge,
...
std::string MLQueryRep::PrepareCondition(const std::string &condition) {
  std::string prepared_condition =
      std::regex_replace(condition, std::regex("[()]"), "");
  prepared_condition.erase(std::remove_if(prepared_condition.begin(),
                                          prepared_condition.end(), isspace),
                           prepared_condition.end());
...
void MLQueryRep::ParsePredicates(JoinHypergraph *graph,
  ...
  for (int pred_idx : BitsSetIn(path->filter_predicates)) {
    ...
    // If there has already been predicates added, and we are iterating the
    // first predicate of path, e.g., already added from AccessPath left_path
    // when we are iterating AccessPath right_path, make sure it continues as
    // comma separated.
...
/**
  ...
  ML model object. This object reads/writes to the ML model through the global
  Ml_socket.
*/
